...
template <typename T = float> struct Vector4 {
  ...
  Vector4(const Vector2<T> &other, T z = 0, T w = 0)
      : x(other.x), y(other.y), z(z), w(w) {}

  Vector4(const Vector3<T> &other, T w = 0)
  ...
  // magnitude
  T magnitude() const { return sqrtl(x * x + y * y + z * z + w * w); }

  T magnitudeSquared() const { return x * x + y * y + z * z + w * w; }

  // negate
  Vector4<T> negate() { return Vector4<T>(x * -1, y * -1, z * -1, w * -1); }
